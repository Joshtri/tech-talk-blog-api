...
import {postPrivateService} from "../../services/post/index.js";
...
const getAllPosts = async (req, res) => {
  try {
    const posts = await postPrivateService.getAllPostsForCMS();
...
const getPostById = async (req, res) => {
  const { id } = req.params;
  try {
    const post = await postPrivateService.getPostByIdForCMS(id);
    if (!post) {
...
const createPost = async (req, res) => {
  try {
    ...
    const newPost = await postPrivateService.createNewPost(postData);
...
const updatePost = async (req, res) => {
  ...
  try {
    ...
    const updatedPost = await postPrivateService.updateExistingPost(id, postData);
    if (!updatedPost) {
      return res.status(404).json({ message: "Post not found" });
    }
...
const deletePost = async (req, res) => {
  ...
  try {
    ...
    const deletedPost = await postPrivateService.deletePostById(id);
    if (!deletedPost) {
      return res.status(404).json({ message: "Post not found" });
    }
